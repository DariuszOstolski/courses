...
object Anagrams {
  ...
  def sentenceOccurrences(s: Sentence): Occurrences = {
    def sentenceToString(s: Sentence): Word = s match {
      case Nil => ""
      case y :: ys => y + sentenceToString(ys)
    }
    wordOccurrences(sentenceToString(s))
  }
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = {
    ...
    dictionary.map((w: Word) => (wordOccurrences(w), w)).groupBy((elem: (Occurrences, Word)) => elem._1).mapValues(reduceOccurences)
  }
