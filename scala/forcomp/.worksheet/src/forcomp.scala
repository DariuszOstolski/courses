object forcomp {
	...
	import forcomp._;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(83); 
  ...
  def wordOccurrences(w: Word): Occurrences = {  w.map(c => c.toLower).groupBy(c => c).mapValues( s => s.length).toList.sortBy(( elem: (Char, Int) )  => elem._1) };System.out.println("""wordOccurrences: (w: forcomp.Word)forcomp.Occurrences""");$skip(36); 
  ...
  };System.out.println("""sentenceToString: (s: forcomp.Sentence)String""");$skip(153); 
  def sentenceOccurrences(s: Sentence): Occurrences = s match {
  	case Nil => List()
  	case y::ys => wordOccurrences(y) ++ sentenceOccurrences(ys)
  };System.out.println("""sentenceOccurrences: (s: forcomp.Sentence)forcomp.Occurrences""");$skip(32); val res$2 = 
  sentenceOccurrences(sentence);System.out.println("""res2: forcomp.Occurrences = """ + $show(res$2))}
